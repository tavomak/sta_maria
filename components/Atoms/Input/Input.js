...
const Input = ({
  ...
  return (
    <div className={phone ? "flex items-center justify-stretch" : ""}>
      {phone && (
        <span className="flex h-[42px] items-center rounded-l-md border-gray-200 bg-gray-100 px-2">
          +56
        </span>
      )}
      <label
        htmlFor={name}
        className={`${phone ? "w-full rounded-r-md" : "rounded-md"} relative my-4 block border border-gray-200 shadow-sm focus-within:border-red-600 focus-within:ring-1 focus-within:ring-red-600`}
      >
        {register ? (
          <input
